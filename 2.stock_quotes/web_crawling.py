...
class StockApp:
    def __init__(self, master):
        ...
        headers = ["序号", "股票代码", "股票名称", "涨跌幅", "当前股价", "当日涨跌"]
        columns = [f"#{i}" for i in range(len(headers))]
    ...
    def update_table(self, i, data, item_dict):
        ...
        if key not in item_dict:
            item = self.tree.insert("", tk.END, text=str(i),
                                    values=(
                                        market_dict[market_code] + stock_code, name, change, price,
                                        change_price),
                                    tags=('green',) if float(change_price) < 0 else (
                                        ('red',) if float(change_price) > 0 else ''))
            item_dict[key] = item
        else:
